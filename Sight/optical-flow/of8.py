...
while True:
    ...
    adams_present = False
    centroid_y = None
    area = cv2.countNonZero((mhi > (0.3 * MHI_MAX)).astype(np.uint8))
    ...
    if adams_present:
        ...
        if delta_norm > HIGH_THRESH and frames_since_last > refractory_frames:
            ...
            if above_counter >= min_above_frames and not swallow_active:
                ...
                # 🔄 Reset centroid & baseline lines after swallow
                centroid_ema = None
                baseline = None
                above_counter = 0
        else:
    ...
    cv2.imshow("Swallow Detector (MHI + centroid + tracker + reset)", frame)
    prev_gray = gray
